    ...
    <table class="table table-bordered">
        ...
        @foreach($users as $user)
            <tr>
                ...
                <td>
                    <a href="{{ route('shops.show',[$user]) }}" class="btn btn-info">查看详细</a>
                    <a href="{{ route('shops.edit',[$user])}}" class="btn btn-warning">重置密码</a>
                    <form style="display: inline" method="post" action="{{ route('shops.destroy',[$user]) }}">
                        {{ csrf_field() }}
                        {{ method_field('delete') }}
